...
- App-level rule support is narrower than `ca-rules2`: the search app currently accepts only 2-state, non-`B0` rules with neighborhood size `<= 24`. Isotropic non-totalistic rules must have a range-1 Moore or hexagonal neighborhood (size `<= 8`), and non-isotropic (MAP) rules must have a range-1 Moore, von Neumann, or hexagonal neighborhood.
